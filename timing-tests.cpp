...
int main() {
    ...
    for (const auto& test : positions) {
        ...
        // auto best_move = best_move_search(pos, globals,test.depth); // Use best_move_search to get the best move


        std::cout << "Best Move: ";
        if (result.pv && !result.pv->empty()) {
            std::cout << result.pv->begin()->to_str();  // Check how to access moves if this is incorrect
        } else {
            std::cout << "N/A";
        }
        std::cout <<  std::endl;
