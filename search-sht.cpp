...
namespace search {
    ...
    search::SearchResult search_impl(libchess::Position& pos, int alpha, int beta, int depth, search::SearchStack* ss, search::SearchGlobals& sg) {
        ...
        #pragma omp parallel
        {
            ...
            for (int i = 0; i < move_list.size(); ++i) {
                ...
                #pragma omp critical
                {
                    if (search_result.score > local_best_score) {
                        local_best_score = search_result.score;
                        if (local_best_score > alpha) {
                            alpha = local_best_score;
                            if (pv_node) {
                                local_pv = search_result.pv.value_or(libchess::MoveList());
                                local_pv.add(move);
                            }
                            if (alpha >= beta) {
    ...
    SearchResult search(Position& pos, int depth) {
        ...
        std::cout << "Duration: " << std::fixed << std::setprecision(3) << duration.count() << " ms" << std::endl;
        std::cout << "Nodes: " << nodes << std::endl;
        std::cout << "NPS: " << std::fixed << std::setprecision(3) << nps << std::endl;
    ...
    std::optional<libchess::Move> best_move_search(libchess::Position& pos, SearchGlobals& search_globals, int max_depth) {
        ...
        for (int depth = 1; depth <= max_depth; ++depth) {
            auto search_result = search(pos, depth);  // Call search defined in search.h
            ...
            libchess::UCIScore uci_score = (score <= -MAX_MATE_SCORE) ?
                libchess::UCIScore((-score - MAX_MATE_SCORE) / 2, libchess::UCIScore::ScoreType::MATE) :
                (score >= MAX_MATE_SCORE) ?
                libchess::UCIScore((score - MAX_MATE_SCORE + 1) / 2, libchess::UCIScore::ScoreType::MATE) :
